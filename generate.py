...
with psycopg2.connect(dsn) as conn:
  with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
    ...
    cur.execute("""WITH buffer AS (
        SELECT ST_Transform(ST_Buffer(ST_Transform(geom, 3857), 5000),4326) AS buffer_geom, geom, plz FROM postcode WHERE plz = '50667' 
      )
      SELECT
        plz,
          ...
          SELECT
            ARRAY_TO_JSON(
              ARRAY_AGG(
                JSON_BUILD_OBJECT(
                  'geom',
                  ST_AsGeoJSON(ST_Simplify(ST_Intersection(buffer.buffer_geom, flood_hazard.geom), 0.00005), 4),
                  'level',
                  qlike
                )
              )
    ...
    data = {
      ...
      'dense_space': {
        'bbox': json.loads(res['dense_space']['bbox']),
        'name': res['dense_space']['name']
      },
    ...
    with gzip.open('temp/postcode_test.gz', 'wt') as f:
